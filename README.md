...
- **Ollama (optional)**: **Describe** (image→prompt, in a **style** you pick), **Improve
  prompt**, and **Vision Mix** (blend several reference images into one prompt). Models
  unload from VRAM after use. Without Ollama, **Describe** uses a local BLIP captioner and
  **Improve prompt** falls back to a local rule-based pass — both work fully offline.
...
- **Describe** (Input Image → Describe): turns an image into a prompt with an Ollama
  ...
  **Prompt AI → Describe style** sets what the model is asked for:
  ...
  | Style | For |
  |---|---|
  | **Prompt (prose)** (default) | rebuilding the image: medium and style first, then subject, clothing, pose, setting with positions, camera, lighting, palette, mood; a sign or title quoted once |
  ...
  **Length** goes from 60 to 300 words, and the accordion under it shows the exact
  ...
  muse-glimmer, each description regenerated by klein at the same seed): naming the medium
  first keeps a drawing a drawing, and a quoted text comes back clean only when it is
  given in one piece. The other styles follow the same rules. A small vision model
...
- **Improve prompt**: rewrites the current prompt via the same Ollama model, keeping its
  form (prose stays prose, tags stay tags). URL + model in Advanced → **Prompt AI**. Tune
  the instructions in `config.txt`. **Without Ollama** it falls back to a local
  rule-based pass that appends quality tags (`improve_local_keywords` in `config.txt`) —
  instant, no model.
...
- **Describe the result** in the prompt (the full outfit/scene). **Auto-describe** runs
  ...
  vision model) to keep the fill coherent with the center — or check it to also prepend a
  description to your prompt.
