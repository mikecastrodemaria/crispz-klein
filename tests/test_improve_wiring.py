...
def test_negative_improve_is_hidden_on_klein():
    import cz_ui
    assert cz_ui.IMPROVE_NEGATIVE_UI is False


def test_think_is_disabled_and_klein_options_travel():
    import cz_ui
    cz_ui._ui_improve("a fox", "llama3.1:8b", URL)
    body = _last()
    assert body["think"] is False
    assert body["options"].get("num_ctx") == cz_ollama.OLLAMA_NUM_CTX or cz_ollama.OLLAMA_NUM_CTX <= 0
