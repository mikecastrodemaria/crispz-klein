"""{a|b|c} variant groups wired into crispz-klein (prompt_variants + cz_prompt).
...
Covers the family rules for the variant syntax:
  ...
  - the negative prompt is expanded too (inert on klein: the protocol clears it);
    ...
    try:
        with _fake_pipeline() as fake:
            spec, w = cp.validate_spec(dict(spec_in, protocol=1, out_dir=d,
                                            width=64, height=64,
                                            detail_faces=False, detail_hands=False))
...
def test_protocol_plain_prompt_is_unchanged():
    ...
    # klein: negative cleared at validation (distilled model, SUPPORTS_NEGATIVE=False)
    assert fake.calls["gen"] == ("a cat", 42, "")
    assert fake.calls["loras"] == [("ink", 0.5)]
    assert res["seed_used"] == 42
    assert any("'negative' ignored" in w for w in res["warnings"]), res["warnings"]
...
def test_protocol_expands_and_reads_only_the_picked_lora():
    ...
    assert (prompt, seed, negative) == ("a cat, park", 7, "")
    assert fake.calls["loras"] == [("catlora", 0.8)]
    assert res["loras"] == ["catlora:0.8"]


def test_protocol_expand_spec_expands_the_negative_it_is_given():
    """validate_spec clears the negative on klein; _expand_spec still expands and
    cleans one if a caller passes it along."""
    import cz_protocol as cp
    with _wildcards({}, in_order=True):
        out = cp._expand_spec({"prompt": "a {cat|dog}", "negative": "{blurry <lora:x>|noisy}",
                               "loras": []}, 3)
    assert out["prompt"] == "a cat" and out["negative"] == "blurry"
    assert out["loras"] == []


def test_protocol_only_lora_tags_after_expansion_is_refused():
    import cz_protocol as cp
    with _wildcards({}, in_order=True):
        try:
            _run_protocol_gen({"prompt": "{<lora:ink>|a cat}", "seed": 1})
        except cp.SpecError as e:
            assert "only <lora:" in str(e)
        else:
            raise AssertionError("empty prompt after expansion must be refused")
...
def _ui_txt2img(prompt, negative, n, seed):
    ...
    try:
        _ui_call(prompt, negative, n, seed)
    finally:
...
def test_ui_omni_resolves_seed_and_expands():
    import cz_ui
    import cz_pipeline
    if not (cz_pipeline.OMNI_MODEL or "").strip():
        print("SKIP test_ui_omni_resolves_seed_and_expands (no omni model)")
        return
    got = {}

    def fake_generate_omni(refs, prompt, negative, width, height, steps, seed, **kw):
        got.update(prompt=prompt, negative=negative, seed=seed, refs=len(refs))
    ...
    try:
        with _wildcards({}, in_order=True):
            _ui_call("a {red|blue} car", "{blurry|noisy}", 1, -1, use_input=True,
                     input_mode="Reference (Omni)", ref1=Image.new("RGB", (32, 32)))
    finally:
        cz_ui.generate_omni = real
    assert got, "generate_omni was not reached"
...
def test_cli_txt2img_resolves_seed_and_expands():
    ...
    assert rc in (0, None)
    assert got["prompt"] == "a cat" and got["negative"] == "blurry", got
    assert got["seed"] >= 0
