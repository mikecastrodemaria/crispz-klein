...
def _te_hint():
    ...
    repo de base courant (releve le 2026-09-10: deux Qwen3-4B telecharges, klein 9B charge,
    liste reduite a "Default" sans un mot)."""
    try:
    ...
    made = [getattr(cz_pipeline, "_ENCODER_WIDTH_FOR", {}).get(s) for s in sizes]
    ...
    return (f"ℹ️ {len(other)} text encoder(s) in the HF cache are hidden here: "
            ...
            f"{width}. {names}"
            + (f" -- made for {', '.join(made)}: pick that base or one of its checkpoints "
               f"to use them." if made else ". Pick a base of their size to use them."))
...
def _vram_hint(e):
    ...
    return ("  \n**VRAM saturee**, meme apres vidage du cache : ferme les autres apps GPU "
            ...
            "references. Si le rendu suivant echoue encore, redemarre crispz-klein.")
    ...
    try:
        ...
        if use_input and input_mode == "Reference (Omni)" and omni_ready:
            ...
            for i in range(n):
                ...
                # Entre deux images, le cache de torch revient au pilote: les detecteurs
                # onnxruntime et les noyaux charges a la demande n'y ont pas acces, et c'est
                # a la 4e image d'un lot Omni + upscale + detaileur que la carte etait pleine.
                        ...
                        with gr.Accordion("\U0001F4E6 Checkpoints (switch model)", open=True):
                            ...
                            with gr.Column():
                                te_dd = gr.Dropdown(
                                    ...
                                    info="Default: the base repo's own Qwen3. Or a transformers "
                                         "folder (config.json + weights) or HF repo of the SAME "
                                         "size, e.g. an abliterated Qwen3. Pick or paste a path; "
