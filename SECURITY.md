...
crispz-klein is developed on `main` with no maintenance branches: only the **latest
commit on `main`** receives fixes. The current version is in `cz_core.py`
(`APP_VERSION`) and shown in the browser tab title.

## Scope

crispz-klein is a **local desktop application**. `build_ui().launch()`
...
rather than vulnerabilities in crispz-klein.
...
that distinction meaningful: crispz-klein is a local app bound to 127.0.0.1, and the
