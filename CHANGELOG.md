...
returns `seed_used`. The negative is expanded where it is still passed along; it stays
inert on this model.
...
description regenerated by klein 4B at the same seed), this time against the "Dataset
paragraph" caption of Captionz: both name the medium, but asking for the **era** raises the
portrait's fidelity from 0.54 to 0.65-0.66 on both models, so the default style now asks for
it next to the time of day and the weather. Captionz's caption rebuilds a scene less well
(0.28 against 0.32: it asks for neither framing, nor lighting, nor positions), but it makes a
good training caption, so it joins the list as the **Dataset paragraph** style: subjects,
objects, setting, era, medium and technique, style and mood, then the aspect ratio.
